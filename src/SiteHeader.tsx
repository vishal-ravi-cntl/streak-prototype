  ...
  return <div className="fixed inset-0 z-[100] grid place-items-center overflow-y-auto bg-black/35 px-6 py-6 backdrop-blur-sm" role="presentation">
    <section className="relative h-[640px] max-h-[calc(100vh-48px)] w-[480px] overflow-hidden border border-[#dedede] bg-white p-2 text-[#111] shadow-[0_10px_26px_rgba(0,0,0,.2)]" role="dialog" aria-modal="true" aria-labelledby="profile-title" onMouseDown={(event) => event.stopPropagation()}>
      <div className="flex h-full min-h-0 flex-col border border-[#eeeeee] px-9 pb-7 pt-8">
        ...
        <div className="mt-[34px] border-y border-[#ededed] py-4"><h3 className="font-tny-irvin text-[23px] font-normal uppercase leading-none">Ways to Earn XP</h3></div>
        <div className="hide-scrollbar min-h-0 flex-1 overflow-y-auto">{rewards.map((reward, index) => <div className={`grid grid-cols-[1fr_auto] items-center py-[14px] ${index < rewards.length - 1 ? 'border-b border-[#ededed]' : ''}`} key={reward.title}>
          <div>{reward.gameLink ? <a className="inline-flex items-center gap-1 text-[14px] font-bold leading-none hover:opacity-70" href="/crossword-puzzles-and-games" onClick={onGames ? (event) => { event.preventDefault(); onGames(); onClose() } : undefined}>{reward.title}<span className="text-[#d93636]"><ExternalLinkIcon /></span></a> : <h4 className={`text-[14px] font-bold leading-none ${reward.status === 'Done' ? 'text-[#9a9a9a]' : ''}`}>{reward.title}</h4>}<p className={`mt-[6px] text-[13px] leading-none ${reward.status === 'Done' ? 'text-[#b0b0b0]' : 'text-[#8a8a8a]'}`}>{reward.description}</p></div>
          <div className="text-right"><div className={`${reward.xp === '+1 XP' ? 'text-[11px]' : 'text-[9px]'} font-extrabold leading-none ${reward.status === 'Done' ? 'text-[#9a9a9a]' : 'text-[#d93636]'}`}>{reward.xp}</div><div className={`mt-1 text-[11px] font-semibold leading-none ${reward.status === 'Done' ? 'text-[#9a9a9a]' : 'text-[#777]'}`}><span className="mr-1">{reward.icon}</span>{reward.status}</div></div>
        </div>)}</div>
      </div>
    </section>
  </div>
